...
- **Usage quota footer**: session/weekly usage percentages and reset
  countdowns, if you have a compatible scraper script configured (see
  `CLAUDE_QUOTA_BIN` below - the script itself isn't part of this repo).
...
- Linux with `tmux`, PHP 8.1+, Composer, and Docker/Docker Compose.
...
5. The dashboard checks on every load whether Claude Code's `SessionStart`
   ...
   - click it, it's a one-time, safe merge into your existing settings.
   Without these, transcript tracking can silently go stale after a
   `/clear`/`/compact`/`--resume`, and a blocked prompt's preview can come
   out truncated for a large command - see
   [CONTRIBUTING.md](CONTRIBUTING.md) for why each hook exists.
