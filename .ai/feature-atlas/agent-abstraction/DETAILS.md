...
- **Isolation** — sets `HOME_ROOT` to a temp fixture home (`:33-34`) and **refuses to run** if `Config::home_root()` still resolves to the real host home directory (`:36-39`). Cleanup in the `finally` block unlinks fixture `settings.json`/`hooks.json` and temp dirs (`:195-202`).
