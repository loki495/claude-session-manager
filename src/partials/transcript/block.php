...
?>
<?php if ($kind === 'text'): ?><div class="copy-block" data-line="<?= (int)$line ?>"><p class="copy-source whitespace-pre-wrap break-words text-sm lg:text-base text-slate-100"><?= $this->e($text) ?></p><button type="button" class="copy-btn select-none text-[11px] text-slate-500 active:text-slate-300 mt-0.5">Copy</button></div>
...
<?php elseif ($kind === 'tool_use'): ?><div class="tool-use-block<?= $subagentClass ?>" data-line="<?= (int)$line ?>"><?= $collapsibleHtml ?></div>
